...
export default function CreatePage() {
  ...
  async function submit() {
    ...
    try {
      ...
      toast(e instanceof Error ? e.message : "Could not create goal", "error");
    } finally {
      setLoading(false);
    }
  }
    ...
    return (
      <main className="mx-auto flex min-h-dvh max-w-md flex-col px-6 pb-10 pt-6">
        <header className="flex items-center gap-3">
          <BackButton />
          <span className="font-display text-lg font-semibold text-ink">All set</span>
        </header>
        <div className="rise-in mt-8 flex flex-1 flex-col items-center text-center">
          <CoinJar pct={0} size={180} face="happy" />
          <h1 className="mt-4 font-display text-[26px] font-semibold text-ink">Goal created!</h1>
          <p className="mt-2 max-w-xs font-medium text-ink-soft">
            Share this link so your friends can chip in too.
          </p>
          ...
          <Link href={`/g/${createdSlug}`} className="mt-3 w-full">
            <PillButton className="w-full">Open goal →</PillButton>
          </Link>
        </div>
      </main>
    );
  ...
  return (
    <main className="mx-auto flex min-h-dvh max-w-md flex-col px-6 pb-10 pt-6">
      <header className="flex items-center gap-3">
        <BackButton />
        <span className="font-display text-lg font-semibold text-ink">New goal</span>
      </header>

      <div className="mt-7">
        <StepDots total={2} current={targetOk ? 1 : 0} />
        <h1 className="mt-5 font-display text-[28px] font-semibold tracking-tight text-ink">Start a new goal</h1>
        <p className="mt-1 font-medium text-ink-soft">Give it a name and a target. We handle the rest.</p>
      </div>

      <div className="mt-6 flex flex-col gap-6">
        <label className="block">
          <span className="mb-2 block text-sm font-bold text-ink-soft">Goal name</span>
          <input
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder="Bali trip with the crew"
            maxLength={60}
          ...
          />
          {touched && !nameOk && (
            <p className="mt-1.5 text-xs font-semibold text-blush">Name needs at least {MIN_NAME} characters.</p>
          )}
        </label>
        ...
        <label className="block">
          ...
          {touched && !targetOk ? (
            <p className="mt-1.5 text-xs font-semibold text-blush">Minimum target is {money(MIN_TARGET)}.</p>
          ) : (
      ...
      <div className="mt-auto pt-8">
        <PillButton onClick={submit} loading={loading} className="w-full py-4 text-base">
          Create goal <span aria-hidden>→</span>
        </PillButton>
        <p className="mt-3 text-center text-xs text-ink-soft/80">
          The jar is created automatically. No wallet, no hidden fees.
        </p>
      </div>
    </main>
  );
